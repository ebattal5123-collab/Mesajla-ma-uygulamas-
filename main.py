...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .profile-btn, .inbox-btn, .admin-panel-btn {
            margin-top: 10px;
            padding: 8px 12px;
            background: #667eea;
            color: white;
        ...
        .profile-btn:hover, .inbox-btn:hover, .admin-panel-btn:hover {
            background: #764ba2;
        }
        .inbox-btn {
            background: #e74c3c;
        ...
        @media (max-width: 768px) {
            ...
            .auth-box, .profile-box, .group-box, .inbox-box, .admin-box { min-width: 90%; }
    ...
    <div class="admin-modal" id="adminModal">
        <div class="admin-box">
            <div class="admin-header">
                <h2 style="color: #f39c12;">👑 Admin Paneli</h2>
                <p style="color: #666; font-size: 14px;">Kullanıcı yönetimi</p>
            ...
            <div class="input-area">
                <div class="file-upload-area">
                    <div class="file-input-wrapper">
                        <input type="file" class="file-input" id="fileInput" multiple accept="image/*,video/*,.pdf,.doc,.docx,.txt,.zip,.rar">
    ...
    <script>
        ...
        function loadAdminUsers() {
            ...
            .then(users => {
                ...
                if (userCountElement) {
                    userCountElement.textContent = `Toplam Kullanıcı: ${users.length}`;
                }
                
                if (users.length === 0) {
                    usersList.innerHTML = '<div class="empty-state"><div style="font-size: 48px; margin-bottom: 15px;">👥</div><p>Kullanıcı bulunamadı</p></div>';
                    return;
                }
                ...
                users.forEach(user => {
                    ...
                    userItem.innerHTML = `
                        <div class="user-details">
                            ...
                            <div class="user-email">📧 Email: ${user.email || 'Bilinmiyor'}</div>
                            <div class="user-created">📅 Kayıt: ${createdDate}</div>
                        </div>
        ...
        function initSocket() {
            ...
            socket.on('receive_message', data => {
                if (data.room === currentRoom) {
                    displayMessage(data.username, data.message, data.timestamp, data.files);
                }
            });
        ...
        function loadMessages(roomName) {
            ...
            .then(messages => {
                ...
                if (messages.length === 0) {
                    ...
                    messages.forEach(msg => displayMessage(msg.username, msg.message, msg.timestamp, msg.files || [], true));
                }
                scrollToBottom();
            });
        }
        
        function displayMessage(user, message, timestamp, files = [], isHistory = false) {
            const messagesDiv = document.getElementById('messages');
...
def login():
    try:
        ...
        if user and is_user_banned(user['user_id']):
...
def get_admin_users():
    try:
        ...
        # Tüm kullanıcıları getir, password hariç
        ...
        for user in users:
            user['banned'] = is_user_banned(user['user_id'])
